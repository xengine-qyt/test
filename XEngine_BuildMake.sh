...
m_EnvAuthBreak=0
m_EnvRelease=0
m_EnvRPM='git gcc-c++ gdb make openssl-devel libcurl-devel mysql-devel zlib-devel minizip-devel mongo-c-driver-devel libpq-devel libsqlite3x-devel libnghttp2-devel'
...
function InstallEnv_CheckIns()
{
	...
	if [ "$m_EnvRelease" -eq "2" ] ; then
		...
		if [ "$VERSION_ID" == "22.04" ]; then
    		m_EnvAPT="$m_EnvAPT wget nasm libsrt-gnutls-dev libspeex-dev libsnappy-dev libsoxr-dev libopenmpt-dev libmodplug-dev libdav1d-dev libass-devel libaom-dev libx264-dev libx265-dev libfontconfig-dev libfreetype-dev libfribidi-dev libharfbuzz-dev libgme-dev libgmp-dev  libmp3lame-dev libopus-dev libxvidcore-dev libsdl2-dev libzip-dev"
